...
python ml/routing/scripts/serialize_runtime_graph.py  # promotes graph.json + metadata.json
```
...
- `graph.json` — plain `{node_coords: [[lon, lat], ...], adjacency: [[[neighbor_id, weight_m], ...], ...]}`.
...
- `metadata.json` — sha256/byte count (verified by `api/_shared/graph_loader.py`
  before load, same trust contract as the ML models), node/edge counts, bbox,
