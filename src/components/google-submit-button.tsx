...
export function GoogleSubmitButton() {
  ...
  return (
    <button
      ...
      disabled={pending}
      aria-busy={pending}
      className="flex w-full items-center justify-center gap-2.5 rounded-xl border border-border bg-surface py-3 text-sm font-medium text-text transition-colors hover:border-text-tertiary disabled:opacity-60"
    >
      {pending ? (
        <Spinner className="h-4 w-4 text-text" />
      ) : (
