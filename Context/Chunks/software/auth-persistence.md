...
Supabase supplies cookie-based auth plus per-user profiles, saved places,
recent searches, and completed walks. Server actions authenticate the caller
before mutations; migrations enable row-level security and owner-scoped
policies. (`src/lib/actions/`, `supabase/migrations/`)

## Key files

- `src/proxy.ts` - session refresh, host routing, public/auth redirects, and
...
- Home and work are single slots per user; favorites are not slot-limited.
...
- `src/proxy.ts` treats unknown unauthenticated app paths as protected and sends
  them to login; signed-in users reach the application 404.
